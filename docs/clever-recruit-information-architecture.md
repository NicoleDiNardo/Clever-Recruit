...
Administrator
├── /admin/users              User list, invite, role change [Implemented]
├── /admin/roles              Roles & permissions reference (read-only — role change stays on the user record at /admin/users) [Implemented]
└── /admin/organisation       Org settings (name, logo — minimal) [Implemented]
...
### `/admin/users`, `/admin/roles`, `/admin/organisation` [Implemented]
**Doc correction**: this section was still marked `[New scope]` even though `pages/Admin/Users/index.tsx` (invite, role change, sole-admin-demotion guard, gated by `RequireRole roles={['admin']}`) and `context/OrgUsersContext.tsx` (invite/changeRole logic, Context+localStorage like the other entities) both already exist and are wired into `App.tsx` at `/admin/users` — this was done in an earlier phase of the engagement (commit `e7361a2`) and the doc was simply never updated to match. Corrected here while assembling the case study, rather than left to misstate what's actually live.
...
`/admin/roles` and `/admin/organisation` were the two items still genuinely unbuilt after that correction — added now. `/admin/roles` is a read-only reference table built from `usePermissions.ts`'s own `ROLE_PERMISSIONS` map (so it can't drift from what `RequireRole` actually enforces), with a link back to `/admin/users` for the real change control — the IA was explicit this shouldn't become a second matrix editor. `/admin/organisation` is deliberately minimal (name + logo, via a new `OrgSettingsContext`, same Context+localStorage shape as `OrgUsersContext`) — a full org-settings/audit-log surface stays [Future scope] per the product definition. All three routes now sit behind one shared `AdminTabs` bar rather than three separate sidebar entries.
